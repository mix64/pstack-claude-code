...
```
/plugin marketplace add C:\Users\mic\claude-plugins
/plugin install pstack@local-plugins
```
...
Upstream runs `arena`, `architect`, and `interrogate` across Claude, GPT, and Grok on purpose. The default here is Claude only (`opus, opus, sonnet`). To bring back other vendors, point panel roles at custom agents in `/pstack:setup-pstack`, for example:

```
interrogate reviewers: opus, agent:ocx-gpt-5-6-luna, agent:ocx-deepseek-v4-flash
```
