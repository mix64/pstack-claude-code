...
This skill orchestrates three others: an inline mining pass (see step 1), the `skill-creator` skill (authoring, when available), and the **unslop** skill (prose discipline). It sequences them. It doesn't replace them.
...
Locate the active workspace's transcripts before fanning out. They live in `~/.claude/projects/<slug>/`, where `<slug>` is the working directory's absolute path with every non-alphanumeric character replaced by `-` (so `C:\Users\you\proj` becomes `C--Users-you-proj`). Each session is `<session-id>.jsonl`, and subagent transcripts sit under `<session-id>/subagents/`. Use only that directory. Don't glob across `~/.claude/projects/*/`. That crosses workspace boundaries and reads private chats from unrelated projects.
...
Use the `skill-creator` skill to author the skill when it is available. Otherwise follow the **Authoring a skill** playbook in `<pstack>/skills/poteto-mode/playbooks/authoring-a-skill.md`. Placement:
