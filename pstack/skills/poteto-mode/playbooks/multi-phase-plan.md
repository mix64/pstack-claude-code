...
**Control skill.** Pick it by surface. Browser, Electron, and web UIs use the built-in browser tools. CLIs and TUIs use Bash. The `run` skill launches the app. Native mobile uses whatever simulator-driving skill the repo has. A PR that touches two surfaces gets lanes on both. A surface with no control skill is a risk in Appendix C, and its live block still names how each lane drives it.
...
- [ ] Read these fresh from disk at program start. Re-read them at every tick. `<pstack>` is the installed plugin root.
  - [ ] `<pstack>/skills/poteto-mode/playbooks/<execution playbook>.md`
  - [ ] `<pstack>/skills/swarm/SKILL.md`
  - [ ] `<control skill or driver path>`
  - [ ] `<pstack>/skills/poteto-mode/playbooks/opening-a-pr.md`
  - [ ] `<pstack>/skills/<each other leaf skill the program uses>`
- [ ] Arm the 30-minute audit tick. In a local session, a real terminal `/loop`. In a remote root, a scheduled wake (the `schedule` skill). Never leave the cadence to memory.
...
Each live lane runs in its own worktree at the PR head. Drive through the built-in browser tools or Bash.
