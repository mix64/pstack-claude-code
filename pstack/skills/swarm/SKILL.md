...
4. Pick the worker model from the `swarm workers` line in `~/.claude/pstack-models.md`. If the rule or that line is missing, use `sonnet`. Resolve the value per the value grammar and spawning rules in the **setup-pstack** skill (`opus`/`sonnet`/`haiku`/`fable` set `model`, `inherit` omits it, `agent:<name>` sets `subagent_type`, `codex:<model>` spawns `pstack:codex-bridge`, `openrouter:<id>` spawns `pstack:openrouter-bridge`, `@<alias>` expands first). If it fails to spawn, use the default and say so. For a model race, name each arm's model up front.
